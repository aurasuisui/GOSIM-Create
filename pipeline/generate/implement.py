...
def build_requirement_brief(tree, a11y_index, req_ids: list[str] | None,
    ...
    if extra_hard:
        # 🔴 「落点约束」（PLAN §7 的「硬清单的回灌 + 整串落在同一个元素」裁决三，2026-09-23 拍板）：
        # 判据的靶子常常是**一个正则链**（`getByRole(…, {name: /邮箱.*用户名.*手机号/})`），
        # 整串要落进**同一个元素**的名字/文本里才算兑现；拆成几个词分散在多个元素 = 没兑现
        # （`ctrip` 第一轮 0/125 就是这个形态：三个词都在产物里，但分散在不同元素）。
        # ⚠️ 这句话只在**带 `extra_hard` 的 brief** 里（= 验证闭环那份），对初始生成零影响。
        # **落点约束按族写**（PLAN 裁决五 ②）：命名族 = 同一个元素的可访问名/可见文本；
        # 字段族 = 该字段的 label / placeholder / name。两句话都给，因为注进来的名字
        # 不带族信息（族在 `eval/` 侧算，不进提交包）——宁可多一句，别让模型猜错位置。
        parts.append("**必须逐字兑现**的补充名（**判据的靶子闭包算出来的**，需求文本没写；"
                     "同样按可访问名或可见文本兑现，不得改名；"
                     "**整串必须落在同一个元素的可访问名 / 可见文本里（按序）**——"
                     "拆成几个词分散在多个元素里**不算**兑现；"
                     "若这条其实是**字段名**，则该字段的 `label` / `placeholder` / 输入框的可访问名也算）：")
