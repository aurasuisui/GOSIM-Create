...
**三个 app 已非零**（闸门是"6 个都非零"，`PLAN.md` §7）：
...
- **⏭ 下一步（当面）· 阶梯 ④：开 `prestashop`（86 条，判分最坏 86 分钟）**
  三个非零已到手（`keep` / `bookstack` / `stackoverflow`），顺序按判分成本升序 →
  下一个是 `prestashop`（86），再 `ctrip`（125），最后 `12306`（135）。
  - 流程同前：**②** 定子集（`python eval/subset_closure.py prestashop` 看「入门度」→ 取 helper 最少 +
    闭包最小 + 未覆盖栏干净的那个）→ **把未覆盖栏注入** `PIPELINE_EXTRA_HARD_NAMES`（**不写进仓库**）
    → **③ 预测并落盘**（`predict.txt`，判分**之前**；它是 9/24 的 `B_pred` 来源）→ 判分（后台任务）。
  - ⚠️ **③ 的盲区已两条**（流程不存在 / 字符串可达≠可访问名可达）→ **别拿它当闸门**（阈值待方案复核）。
  - ⚠️ 已知会让 ② 报假阳的一类：通用字段解析器里的**候选**选择器（`css 'textarea'` 那种）——
    在把它当硬要求之前要先分"候选 / 必需"。
