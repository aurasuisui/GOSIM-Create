...
**四个 app 已非零**（闸门是"6 个都非零"，`PLAN.md` §7）：
...
- **⏭ 下一步（当面）· 阶梯 ④ 继续：开 `ctrip`（125 条，判分最坏 125 分钟）**
  四个非零已到手（`keep` / `bookstack` / `stackoverflow` / `prestashop`），按成本升序 →
  下一个是 `ctrip`（125），最后 `12306`（135）。
  - 流程同前：**②** 定子集 → 未覆盖栏注入 `PIPELINE_EXTRA_HARD_NAMES`（不写进仓库）→
    **③ 预测并落盘**（判分前）→ 判分（后台任务、串行）。
  - 🔴 **有一条待方案会话拍板的后续**：把 **③ 的 unreachable 列表回灌生成硬清单**
    （prestashop 那轮实测：需求里的英文术语被译成中文 → 判据找不到原词；现有注入机制能治，
    但它改的是**生成输入** → 属方案层）。**在它落地之前，别指望子集装置把这类失败挡住。**
  - ⚠️ ③ 的盲区两条（流程不存在 / 字符串可达≠可访问名可达）已收窄到"只在可命中位置里找"，
    但**仍是信息不是闸门**（阈值归方案会话）。
  - ⚠️ 跑判分前 `git status` 必须为空；长判分轮**用后台任务**（AGENTS 第 19 条）。
