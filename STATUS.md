...
- [x] **段三.2 keep 最小子集 = 非零**：`PIPELINE_REQ_IDS=REQ-2.1,REQ-2.2` + 官方档：
      ...
      **`REQ-2.1: Note Listing` 通过（339ms）← 主赛道第一条非零**。
      原因已定位：REQ-2.1 的契约全在文本里（种子数据+标题）；**REQ-2.2 需要的
...
- 🔴 **发现（下一步最有价值的杠杆）**：keep 的头号失败 `button[Toggle sidebar]`（14 次断言）
  **在需求文本里根本不存在**（"toggle" 一次都没出现）——它只在参考截图里。
  而 **`requirements/reference/*.png` 就在需求目录里**，**平台会注入 `VISUAL_MODEL`**
  → **图片链路能把"只能从截图知道的名字"变成靶子**（同时顺带验证 A1 的视觉凭据）。
...
0. **✅ M3b-0 已完成**（见「已完成」与 `docs/11`）；**模型/成本标定已完成**（`docs/12`）。
   ...
   三条优先的当前状态（2026-09-21）：
   - ① **止血**：`check_db_tables` 修好并接回闸门（回归 5/5）✅；
     **建表确定化**（`schema.sql` 独立产物 + 管线注入到启动流程）✅ 已实现、自测全绿，
     ⬜ **待第三次跑闸门第 0 条验证**——🔴 **这是卡点：它不过就不要铺开 6 个 app**
   - ② **V-6「非零」口径**：⬜ 选 `keep`（32 条测试，六个里最小）本地迭代到 3–5 条通过 →
     `./eval/package.sh` 出包 → 用户**只跑这一个任务**，看 ① run 是否 `completed`
     ② 榜单是否出现条目（顺带回答"同一 snapshot 能否反复跑任务"）
     ；若 9/24 前覆盖不到 6 个 app → **保深度不铺广度**（一个 app 有分，好过 6 个都是 0 分）
   - ③ **迭代卫生**：✅ review 已按预算分组；⬜ `score_app.sh` 三处（跑前清 sqlite /
     `load_snapshot` + RunRecord / `ERR_CONNECTION_REFUSED` 计数）；
     ⬜ metrics 加 `run_id` + 成本表**由脚本聚合**（同类错数已手抄三次）
